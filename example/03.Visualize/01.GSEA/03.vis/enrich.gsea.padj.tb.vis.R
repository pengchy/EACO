...
#read in data sets
load("/panfs/home/kang/yangpc/bin/EnrichPipeline/EACO_r20150201/example/03.Visualize/01.GSEA/02.cluster/enrich.gsea.padj.tb.logp.tree.RData")
kp <- read.table("/panfs/home/kang/yangpc/bin/EnrichPipeline/EACO_r20150201/example/01.prep/Kappa/gSets.gmt.filt.gmt.kappa.mt",check.names=FALSE)
kp <- 1-kp
colnames(kp) <- toupper(colnames(kp))
rownames(kp) <- toupper(rownames(kp))
etcfg <- read.table("/panfs/home/kang/yangpc/bin/EnrichPipeline/EACO_r20150201/example/02.Enrich/GSEA/Etb.cfg",se="\t")
et.sig.log <- read.table("/panfs/home/kang/yangpc/bin/EnrichPipeline/EACO_r20150201/example/03.Visualize/01.GSEA/enrich.gsea.padj.tb.logp",sep="\t")
...
write.csv(dynamic.tree.dt,file="/panfs/home/kang/yangpc/bin/EnrichPipeline/EACO_r20150201/example/03.Visualize/01.GSEA/03.vis/enrich.gsea.padj.tb.cluster_desc.csv")
...
pdf("/panfs/home/kang/yangpc/bin/EnrichPipeline/EACO_r20150201/example/03.Visualize/01.GSEA/03.vis/enrich.gsea.padj.tb.tree_pairs_indx.pdf",width=10,height=10)
pairs(as.data.frame(dynamic.tree),lower.panel=adjrdi,diag.panel=barp,gap=0)
dev.off()

gs <- rep(etcfg[,3],each=NROW(et.sig.log))
bg <- rep(etcfg[,4],each=NROW(et.sig.log))
time <- rep(etcfg[,5],each=NROW(et.sig.log))
...
go.dt <- data.frame(go.term=rep(rownames(et.sig.log),NCOL(et.sig.log)),
		...
		gs=gs,time=time,bg=bg,dirct=dirct)
go.dt$bg <- factor(go.dt$bg,levels=c("b","g"))
go.dt$gs <- factor(go.dt$gs,levels=c("GI","SC"))
go.dt$time <- factor(go.dt$time,levels=c("4h","8h","16h","32h","64h","C"))

save(hcl.lst.al,dynamic.tree,abr,dst.lst,go.dt,setDes,et.sig.log,file="/panfs/home/kang/yangpc/bin/EnrichPipeline/EACO_r20150201/example/03.Visualize/01.GSEA/03.vis/enrich.gsea.padj.tb.logp.vis.RData")
...
for(i in names(hcl.lst.al)){
  ...
  pdf(file=paste("/panfs/home/kang/yangpc/bin/EnrichPipeline/EACO_r20150201/example/03.Visualize/01.GSEA/03.vis/enrich_logp_gsets_hcl.",i,".pdf",sep=""),
			height=dim(et.sig.log)[1]/6,width=dim(et.sig.log)[2]/2)
	grid.draw(p.bar.grb)
	#print(p.bar)
	dev.off()
